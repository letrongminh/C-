...
static size_t pages_count(block_size mem) {
    return mem.bytes / getpagesize() + ((mem.bytes % getpagesize()) > 0);
}

static size_t round_pages(block_size mem) { return getpagesize() * pages_count(mem); }
...
static void block_init(void *restrict addr, const block_capacity block_sz, void *restrict next) {
...
static block_size region_actual_size(const block_size query) { return size_max(round_pages(query), REGION_MIN_SIZE); }
...
static bool block_splittable(const struct block_header *const restrict block, const block_capacity query_capacity) {
    return block->is_free && ((size_from_capacity((block_capacity){query_capacity.bytes + BLOCK_MIN_CAPACITY})).bytes <= block->capacity.bytes);
}
...
static struct block_search_result find_good_or_last(struct block_header *restrict block, const block_capacity sz ) {
    while (block->next != NULL) {
        while (try_merge_with_next(block));
        if (block_splittable(block, sz ))
            return (struct block_search_result) {.type = BSR_FOUND_GOOD_BLOCK, .block = block};
        block = block->next;
    }
    return (block_splittable(block, sz ))
...
static struct block_search_result try_memalloc_existing(struct block_header *restrict block, const block_capacity query) {

    struct block_search_result splittable_or_last_block = find_good_or_last(block, query);
    if (splittable_or_last_block.type != BSR_FOUND_GOOD_BLOCK) return splittable_or_last_block;
    split_if_too_big(splittable_or_last_block.block, query);

    return splittable_or_last_block;
}


static struct block_header* grow_heap(struct block_header *restrict last, const block_size query) {
...
static struct block_header* memalloc(struct block_header *restrict const heap_start, const block_capacity query) {
    struct block_search_result block_search_result = try_memalloc_existing(heap_start, query);
    if (block_search_result.type) {
        const block_size query_size = size_from_capacity(query);
        struct block_header *new_block = grow_heap(block_search_result.block, query_size);
        if (new_block == NULL) return NULL;
        block_search_result = try_memalloc_existing(heap_start, query);
    }
